...
require_once('include/registro_cobroXBI.php');

$v = isset($_GET['v']) ? $_GET['v'] : "1";
$c = isset($_GET['c']) ? $_GET['c'] : header("Location: ../rt/") && die();
//$t = isset($_GET['t']) ? $_GET['t'] : header("Location: ../rt/") && die();
//$f = isset($_GET['f']) ? $_GET['f'] : header("Location: ../rt/") && die();
...
function shootLink($NUMBER,$CODIGO,$VERIFICATION){
	...
	if($rptsus['EstaSuscritoResult'] == '0') {
	    header("Location: http://m.tuyonextel.com.pe/suscribete.php?serv=_RTWAPNX");exit;
	}

	//$cobroXBI = new cobroNextel;
	//$sqn = $cobroXBI -> getSecuenciaCobroXBI($idSrv, $nu, $idcontenido,$desSrv);
	$params = array(
        'token'  => "M0b1l3.",
        'idSrv'  => 124,
        'desSrv' => 'RealTone',
	...
	if ($RespuestaXBI['CobroXBIResult'] == "0") {
		...
		$cadena = '&nue='.$NUMBER;
		header('Location: index.php?error=007'.$cadena);exit;
	}
} else {
...
if (isset($_GET['nue'])) {
	...
	if (( strlen($number) == 11)) {
		if (substr($number, 0, 2) == "51") {
			shootLink2($number, $c, "0");
		}
	} elseif (strlen($number) == 9) {
		if (substr($number, 0, 2) !== "51") {
			shootLink2("51" . $number, $c, "0");
		}
	}
}
...
<link type="image/x-icon" rel="shortcut icon" href="img/favicon.ico">
	...
	<meta property="og:site_name"
		content="Portal WAP - Nextel  M&uacute;sica" />
	<meta property="og:image"
		content="http://www.moso.pe/img/moso-logo.png" />
	...
	<link rel="stylesheet" media="screen and (max-width: 900px)"
		href="css/pad.css" />
	<link rel="stylesheet"
		media="handheld and (max-width: 400px), screen and (max-device-width: 400px),screen and (max-width: 400px)"
		href="css/mobile.css" />
...
<body>
	<div id="page">
		...
		<div id="list-titulo-precio">Introduce tu n&uacute;mero de nextel y
			haz clic en &quot;Enviar&quot;</div>
		<div id="list-musica" style="padding: 20px; border: solid 0px red;">
			<form name="frm" action="validacion.php" method="get">
				<div style="">
					<input id="id_num" type="text" name="num" class="c-filtro" /><input
						type="hidden" name="c" value="<?= $c; ?>" /><input type="hidden"
						name="v" value="<?= $v; ?>" />
				</div>
				<?php
				echo $body;
