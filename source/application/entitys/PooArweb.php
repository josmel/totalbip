<?php
class Application_Entity_PooArweb {
	...
	function get_fileNowXml($CATEGORIA){
		...
		$doc = new DOMDocument();
		$file=$this->get_fileNowXml("portal");
		//$path =$this->_config["app"]["xmlPortalft2"];
		$doc->load('../xml/ft/'.$file);		
	...
	function getDestacado(){ ;
		...
		foreach( $HEADs as $HEAD ){
			...
			foreach( $DESTACADOs as $DESTACADO ){
				...
				$data[$i]["RUTAIMAGEN"] = str_replace("_ImagenesPortalWap/","ft/destacado/",$RUTAIMAGEN->item(0)->nodeValue);
				$i++;			
	...
	function get_categoria($CATEGORIA,$NUM=3,$TOPTXT=true,$img,$baseurl,$vermas){
		...
		$doc->load( $p_path->getPath('../xml/ft') . '/' . $file );
